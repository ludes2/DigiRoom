...
<script>
import LineChart from './LuxChart.vue'
...
async mounted () {
  ...
  methods: {
    async fetchDataFromThisWeek(){
      await axios.get('/api/luxFromThisWeek/' + this.room.id)
      .then(response => {
    ...
    async fetchDataFromThisMonth(){
      await axios.get('/api/luxFromThisMonth/' + this.room.id)
      .then(response => {
          this.chartData = response.data.map(elem => elem.value);
          this.chartLabels = response.data.map(elem => elem.created_at);
          this.average = this.calculateAverage(this.chartData);
    ...
    async fetchDataFromThisYear(){
      await axios.get('/api/luxFromThisYear/' + this.room.id)
      .then(response => {
          this.chartData = response.data.map(elem => elem.value);
          this.chartLabels = response.data.map(elem => elem.created_at);
          this.average = this.calculateAverage(this.chartData);
    ...
    calculateAverage(array){
      var average = array.reduce((a, b) => parseInt(a) + parseInt(b), 0) / array.length;
