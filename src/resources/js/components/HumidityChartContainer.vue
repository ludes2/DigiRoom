...
<script>
import LineChart from './HumidityChart.vue'
...
export default {
  ...
  methods: {
    async fetchDataFromThisWeek(){
      await axios.get('/api/humiditiesFromThisWeek/' + this.room.id)
      .then(response => {
    ...
    async fetchDataFromThisMonth(){
      await axios.get('/api/humiditiesFromThisMonth/' + this.room.id)
      .then(response => {
          this.chartData = response.data.map(elem => elem.value);
          this.chartLabels = response.data.map(elem => elem.created_at);
          this.average = this.calculateAverage(this.chartData);
    ...
    async fetchDataFromThisYear(){
      await axios.get('/api/humiditiesFromThisYear/' + this.room.id)
      .then(response => {
          this.chartData = response.data.map(elem => elem.value);
          this.chartLabels = response.data.map(elem => elem.created_at);
          this.average = this.calculateAverage(this.chartData);
